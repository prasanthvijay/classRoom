...
<select aria-hidden="true" tabindex="-1" multiple="multiple"  name="employeelist[]" id="employeelist" class="select2 select2-hidden-accessible">
		<?php for($i=0;$i<count($departList);$i++){ 
			...
			$employeeList = $em->createQuery("SELECT j.userid,j.name,j.chorusid FROM TraxAdminBundle:TblUser j  where j.department = '".$departList[$i]['dprtid']."' and j.customerid='".$loginuserId."' and j.employeetype='Trainee'")->getArrayResult();
...
<?php } else if($getFunctionType=='getsubCategory'){ ?>
		...
		<?php }?> 
		</select><div id="errorSubCategory" style="color:red"></div>
